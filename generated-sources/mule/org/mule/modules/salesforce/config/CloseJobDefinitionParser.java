    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private static Logger logger = LoggerFactory.getLogger(CloseJobDefinitionParser.class);

    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(CloseJobMessageProcessor.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the @Processor [close-job] within the connector [sfdc] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [close-job] within the connector [sfdc] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
